...
from weight_graph import WeightedGraph
...
def dijkstra(wg: WeightedGraph[V], root: V) -> Tuple[List[Optional[float]],
    ...
    distances[first] = 0  # root is 0 away from the root
    path_dict: Dict[int, WeightedEdge] = {}  # how we got to each vertex
    pq: PriorityQueue[DijkstraNode] = PriorityQueue()
    pq.push(DijkstraNode(first, 0))

    while not pq.empty:
        u: int = pq.pop().vertex  # explore next closest vertex
        dist_u: float = distances[u]  # should have already seen it
        # look at every edge/vertex from vertex in question
        for we in wg.edges_for_index(u):
            ...
            if dist_v is None or dist_v > we.weight + dist_u:
                ...
                # update the edge on shortest path to this vertex
                path_dict[we.v] = we
                # explore it soon
...
# takes a dictionary of edges to reach each node, returns list
# of edges that goes from start to end
...
if __name__ == '__main__':
    city_graph2: WeightGraph[str] = WeightedGraph(["Seattle", 
    "San Francisco", "Los Angeles", "Riverside", "Phoenix", 
    ...
    distances)
    print('distances from LA:')
    for key, value in  name_distance.items():
        print(f'{key}: {value}')
    print('')  # blank line

    print('shortest path from LA to Boston:')
    path: WeightPath = path_dict_to_path(city_graph2.index_of('Los Angeles'),
