...
class Stack(Generic[T]):
    ...
    @property
    def empty(self) -> bool:
        return not self._container # not is true for the empty container
...
List[T]]) -> Optional[Node[T]]:
    ...
    # keep going while there is room to explore
...
def node_to_path(node: Node[T]) -> List[T]:
    ...
    # work bathwards from end to front
...
def bfs(initial: T, goal_test: Callable[[T], bool], successors: Callable[[T], List[T]]) -> Optional[Node[T]]:
    ...
    # keep going while there is more space to explore
    while not frontier.empty:
        current_node: Node[T] = frontier.pop()
        current_state: T = current_node.state 
        # if we found the goal, we're done
...
List[T]], heuristic: Callable[[T], float]) -> Optional[Node[T]]:
    ...
    # keep going while there is more room to explore
    while not frontier.empty:
        current_node: Node[T] = frontier.pop()
        current_state: T = current_node.state
        # if we found the goal we're done
        ...
        for child in successors(current_state):
            ...
            # assumes a grid, need a cost function for more sophisticated apps
